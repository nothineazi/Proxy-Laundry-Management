          ...
          <div class="box">
            ...
            <div class="box-body">
              ...
              <a href="home.php"><button type="button" class="btn btn-success btn-sm"> 
				...
				<form class="form-horizontal" role="form" id="" action="" method="post" enctype="multipart/form-data">
...
if(isset($_POST['submit'])){
	...
	$saveLaundry =mysqli_query($link, "insert into clothe (type, customer, status) values('$type', '$customer', 'Requested')");
	if($saveLaundry){
		...
		$c_id=$row['id'];
		$sdate=date("y-m-d");
		$save_order=mysqli_query($link, "insert into customer_order (clothe, service, mode, cost, admin, date, status) 
		                                 values ('$c_id', '$service', '$mode', '$cost', '1', '$sdate', 'Requested')");
		if($save_order)
		{
			?>
			<script>
		...
		$mysql=mysqli_query($link, "update payment set outstanding = outstanding + '$cost' where customer='$customer'");
